...
- `models` 数组 — 每条记录包含 `label`、`model`、`baseUrl`、`apiKey`
- `maxTokens` — 全局 token 上限
- `gitRemote` — Git remote 名称

### `data.json`

只保存本地运行偏好：

- `activeModelLabel` — 工具栏当前选中的模型
- 功能开关
- 本地 UI / 运行行为配置
...
所以它适用于 Anthropic 官方接口，以及兼容 Anthropic Messages API 的代理、网关或中转服务。
它默认不兼容 OpenAI Chat Completions、OpenAI Responses、Gemini、Ollama 或其他自定义协议，除非对方明确做了 Anthropic 格式兼容。
